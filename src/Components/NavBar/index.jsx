...
const NavBar = () => {
  ...
  const renderNavBar = () => {
    if (isClickable) {
      return (
        <nav className="relative flex">
          ...
          <ul
            ...
            <li className={`${showCategories ? "flex" : "hidden"}`}>
              <NavLink
                to="/"
                onClick={() => setSearchByCategory()}
                className={({ isActive }) =>
                  isActive ? activeStyle : "text-sm"
                }>
                All
              </NavLink>
            </li>
      ...
      return (
        <nav className="flex justify-evenly items-center w-screen gap-8 pt-3 px-5 bg-white z-10 fixed top-0">
          <ul className={"flex sm:flex sm:items-center sm:gap-5"}>
            <li className="font-semibold text-3xl">
